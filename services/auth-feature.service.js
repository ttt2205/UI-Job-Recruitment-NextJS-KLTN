...
export const loginWithGoogle = async (token) => {
  try {
    const res = await axiosClient.post(`${API_BACKEND_AUTH}/login-google`, {
      token,
    });
    return res;
  } catch (error) {
    console.error(`Lỗi khi gọi API ${API_BACKEND_AUTH}/login-google:`, error);
    throw error;
  }
};
...
export const registerWithGoogle = async (token, role) => {
  try {
    const res = await axiosClient.post(`${API_BACKEND_AUTH}/register-google`, {
      token,
      role,
    });
    return res;
    ...
    console.error(
      `Lỗi khi gọi API ${API_BACKEND_AUTH}/register-google:`,
      error
    );
    throw error;
  }
